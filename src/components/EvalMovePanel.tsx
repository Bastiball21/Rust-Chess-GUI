...
const EvalMovePanel: React.FC<EvalMovePanelProps> = ({ evalHistory, currentEval, moves }) => {
        ...
        {activeTab === 'eval' && (
          <div className="h-full flex flex-col gap-3">
            <div className="flex items-center justify-between">
              <div>
                <div className="text-xs uppercase tracking-wider text-gray-400">Current eval</div>
                <div className="text-2xl font-bold font-mono text-blue-300">{currentEval}</div>
              </div>
              <div className="text-xs text-gray-400">History</div>
            </div>
            <div className="flex-1 bg-gray-900/60 rounded border border-gray-700 p-2">
              {evalHistory.length === 0 ? (
                ...
                <svg viewBox="0 0 320 140" className="w-full h-full">
                  ...
                  <line x1="0" y1="70" x2="320" y2="70" stroke="#334155" strokeDasharray="4 4" />
                  <polyline fill="none" stroke="url(#evalLine)" strokeWidth="2" points={chart.points} />
                </svg>
              )}
            </div>
          </div>
